-- ============================================================================
-- TapParfum Portaal — COMPLETE DATABASE-INSTALLATIE (alles-in-één, v8)
-- Plak dit VOLLEDIG in Supabase -> SQL Editor -> Run. Veilig om opnieuw te
-- draaien (idempotent). Bevat migraties 001 t/m 019 (nieuw: won/verloren-reden bij
-- deals, winkelvraag-types mijlpaal/bezoek, 'nieuw voor partner'-signaal, en het
-- landelijke Sales Game-klassement).
-- ============================================================================
...
-- ============================================================================
-- Sales Game — landelijk klassement (server-aggregaat, geanonimiseerd) — 019
-- Rekent server-side over ALLE traject-tappunten en geeft alleen een
-- geanonimiseerd klassement terug (andere winkels heten "Winkel #n"). De score
-- spiegelt exact modules/salesgame/logic.js + modules/punten/logic.js — houd de
-- punt-gewichten hieronder in sync met BASIS/BONUS_MANUAL.
-- ============================================================================
...
with base as (
  ...
  where coalesce(tp.data->>'traject', '') <> 'false'
),
calc as (
  select b.*,
