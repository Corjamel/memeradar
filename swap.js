...
/* ---------- snelknoppen (snel kopen/verkopen) ---------- */

function setAmount(v) {
  S('swapAmount').value = String(v).replace('.', ',');
  fetchQuote(); // meteen offerte, geen wachttijd
}

// plat decimaalgetal zonder wetenschappelijke notatie (memecoin-hoeveelheden zijn
// vaak miljoenen; toPrecision/toString zou "9,38e+6" maken en toBaseUnits breken)
function plainAmount(x) {
  if (!isFinite(x) || x <= 0) return '0';
  return x.toLocaleString('en-US', { useGrouping: false, maximumSignificantDigits: 9 });
}

// leest het tokensaldo (verkoop) of het native saldo (koop) uit de wallet
async function walletBalanceTokens(row, side) {
  if (row.chainId === 'solana') {
    if (side === 'sell') {
      const dec = swap.decimals ?? await getSolDecimals(row.address);
      const res = await fetch(SOL_RPC, {
        method: 'POST', headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'getTokenAccountsByOwner',
          params: [state.wallets.sol, { mint: row.address }, { encoding: 'jsonParsed' }] }),
      });
      const d = await res.json();
      let raw = 0n;
      for (const a of d.result?.value || []) raw += BigInt(a.account.data.parsed.info.tokenAmount.amount);
      return Number(raw) / 10 ** dec;
    }
    const res = await fetch(SOL_RPC, {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'getBalance', params: [state.wallets.sol] }),
    });
    const d = await res.json();
    return Math.max(0, (d.result?.value || 0) / 1e9 - 0.01); // reserve voor kosten
  }
  // EVM — eerst naar de juiste keten wisselen, anders lezen we saldo/decimalen
  // van het verkeerde netwerk (en vergiftigen we de decimals-cache)
  await ensureChain(row.chainId);
  const wallet = state.wallets.evm;
  if (side === 'sell') {
    const dec = await getEvmDecimals(row.chainId, row.address);
    const data = '0x70a08231' + wallet.slice(2).toLowerCase().padStart(64, '0');
    const hex = await window.ethereum.request({ method: 'eth_call', params: [{ to: row.address, data }, 'latest'] });
    return Number(BigInt(hex)) / 10 ** dec;
  }
  const hex = await window.ethereum.request({ method: 'eth_getBalance', params: [wallet, 'latest'] });
  return Math.max(0, Number(BigInt(hex)) / 1e18 - 0.002); // reserve voor gas
}

function renderQuickAmts(row, side) {
  const box = S('quickAmts');
  const connected = !!walletFor(row);
  if (side === 'buy') {
    const sym = nativeSymbol(row);
    const presets = { SOL: [0.05, 0.1, 0.25, 0.5], ETH: [0.005, 0.01, 0.025, 0.05],
      BNB: [0.02, 0.05, 0.1, 0.25], POL: [5, 10, 25, 50], AVAX: [0.2, 0.5, 1, 2] };
    const vals = presets[sym] || [0.05, 0.1, 0.25, 0.5];
    box.innerHTML = vals.map((v) =>
      `<button class="quick-chip" data-amt="${v}">${String(v).replace('.', ',')} ${sym}</button>`).join('')
      + (connected ? '<button class="quick-chip max" data-pct="100">MAX</button>' : '');
  } else {
    box.innerHTML = [25, 50, 100].map((p) =>
      `<button class="quick-chip" data-pct="${p}">${p === 100 ? 'Alles' : p + '%'}</button>`).join('');
  }
  box.querySelectorAll('.quick-chip').forEach((btn) => {
    btn.addEventListener('click', async () => {
      if (btn.dataset.amt) { setAmount(btn.dataset.amt); return; }
      if (!walletFor(row)) { setStatus('Verbind eerst je wallet om je saldo te gebruiken.', 'err'); return; }
      setStatus('Saldo ophalen…');
      try {
        const bal = await walletBalanceTokens(row, side);
        const pct = parseInt(btn.dataset.pct, 10);
        // 100% ("Alles"): minieme marge zodat afronding/koersdrift nooit boven het
        // saldo uitkomt (dat zou de transactie laten falen)
        let amt = bal * (pct / 100) * (pct >= 100 ? 0.999 : 1);
        if (amt <= 0) { setStatus('Geen saldo gevonden voor dit token.', 'err'); return; }
        setStatus('');
        S('swapAmount').value = plainAmount(amt).replace('.', ',');
        fetchQuote();
      } catch (e) { setStatus(`Saldo ophalen mislukt: ${escapeHtml(e.message || e)}`, 'err'); }
    });
  });
}
...
window.openSwap = async function openSwap(key, side) {
  ...
  swap.opener = document.activeElement; // voor focus-herstel bij sluiten
...
function closeSwap() {
  ...
  // focus terug naar het element dat de modal opende (toegankelijkheid)
  if (swap.opener && document.contains(swap.opener)) swap.opener.focus();
  swap.opener = null;
}
...
function maybeQuote() {
  clearTimeout(swap.debounce);
  swap.debounce = setTimeout(fetchQuote, 250); // sneller reageren tijdens typen
}
...
async function executeSwap() {
  ...
  try {
    ...
    // geslaagd: offerte is verbruikt → knop niet opnieuw activeren (geen dubbele swap)
    swap.quote = null;
  } catch (e) {
