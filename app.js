...
const CHAINS = [
  ...
  // chains die GeckoTerminal niet indexeert → alleen via DexScreener
  { gt: null,          label: 'Robinhood', ds: 'robinhood', dexOnly: true },
];
const GT_NETS = CHAINS.filter((c) => c.gt).map((c) => c.gt);
const GT_TO_CHAIN = Object.fromEntries(CHAINS.filter((c) => c.gt).map((c) => [c.gt, c.ds]));
// waarde in het chain-menu: GeckoTerminal-id, of het DexScreener-id voor dex-only chains
const chainValue = (c) => c.gt || c.ds;
function isDexOnly(chain) { return CHAINS.some((c) => c.dexOnly && c.ds === chain); }
...
// tokens van één DexScreener-chain (bv. robinhood, die GeckoTerminal niet kent)
async function loadDexChain(dsChain) {
  const queries = [dsChain, 'sol', 'usdc', 'pepe', 'ai']; // brede netten → meer treffers
  const all = await Promise.allSettled(
    queries.map((q) => fetchJson(`${DS}/latest/dex/search?q=${encodeURIComponent(q)}`)
      .then((d) => bestPairPerToken(d.pairs).map(mapDexPair).filter((r) => r.chainId === dsChain)))
  );
  const rows = dedupeByToken(all.flatMap((r) => (r.status === 'fulfilled' ? r.value : [])))
    .filter((r) => (r.liquidity ?? 0) > 500);
  if (!rows.length) throw new Error(`geen ${dsChain}-tokens gevonden`);
  return rows;
}
...
async function loadDexTrending() {
  ...
  // DexScreener heeft geen trending-endpoint; we halen per chain brede zoekresultaten
  // en houden de tokens van die chain over.
  const all = await Promise.allSettled(
...
const loadStijgers = () => isDexOnly(state.chain)
  ? loadDexChain(state.chain).then((rows) => rows.filter((r) => (r.change24h ?? 0) > 0))
  : withDexFallback(() => loadGtTab('pools?sort=h24_volume_usd_desc', 2)
...
function render() {
  ...
  if (!rows.length) {
    ...
    empty.textContent = state.tab === 'watch'
      ...
      : state.tab === 'search'
        ? (state.searchQuery
            ? `Geen tokens gevonden voor "${state.searchQuery}". Probeer een andere naam of het contractadres.`
...
async function renderBotPanel() {
  ...
  let s;
  let isDemo = false;
  if (!panel.dataset.loaded) panel.innerHTML = '<div class="bot-card"><p class="token-sub">Bot-gegevens laden…</p></div>';
  try {
...
function init() {
  ...
  for (const c of CHAINS) {
    ...
    o.value = chainValue(c); o.textContent = c.label + (c.dexOnly ? ' ⚠️' : '');
    sel.appendChild(o);
  }
  ...
  $('currency').addEventListener('change', async () => {
    ...
    saveSettings();
    if (state.currency === 'eur' && eurRateStale()) await fetchEurRate();
    if (state.currency === 'eur' && state.usdToEur == null) {
      const box = $('errorBox');
      box.textContent = 'ℹ️ EUR-koers even niet beschikbaar — prijzen worden tijdelijk in USD ($) getoond.';
      box.classList.remove('hidden');
    } else {
      $('errorBox').classList.add('hidden');
    }
