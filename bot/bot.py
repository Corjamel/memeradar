...
def auto_scan_mints(cfg):
    """Zoek gevestigde, veelverhandelde Solana-tokens (hoogste volume) die
    aan alle risicofilters voldoen. Dit zijn echte, geverifieerde tokens —
    geen gloednieuwe boosted pump.fun-tokens — zodat de bot veilig kan leren."""
    addrs = []
    for page in (1, 2):
        try:
            d = http_json(f"{GT}/networks/solana/pools"
                          f"?sort=h24_volume_usd_desc&include=base_token&page={page}")
        ...
        for p in d.get("data", []):
            rel = (p.get("relationships", {}).get("base_token", {})
                   .get("data", {}).get("id", ""))
            if "_" in rel:
                addr = rel.split("_", 1)[1]
                if addr not in addrs:
                    addrs.append(addr)
    if not addrs:
        return []
    market = fetch_market(addrs[:40])
    # alleen stijgers die door alle filters komen (geverifieerd, liquide, oud genoeg)
    ok = [m for m, d in market.items()
          if (d.get("change24h") or 0) > 0 and passes_filters(d, cfg, from_scan=True)]
    return ok[:10]
...
def load_state(cfg):
    if os.path.exists(STATE_PATH):
        with open(STATE_PATH) as f:
            st = json.load(f)
        # paper en live NOOIT dezelfde portefeuilles laten delen: bij een
        # moduswissel met een verse staat beginnen (nepgeld ≠ echt geld)
        if st.get("mode") not in (None, cfg.get("mode")):
            st = None
        if st and "variants" in st:
            for v in cfg.get("training", {}).get("varianten", []):
                st["variants"].setdefault(v["naam"], new_portfolio(cfg))
            # ontbrekende sleutels in bestaande portefeuilles aanvullen (migratie)
            defaults = new_portfolio(cfg)
            for port in st["variants"].values():
                for k, dv in defaults.items():
                    port.setdefault(k, dv if not isinstance(dv, (dict, list)) else type(dv)())
            st.setdefault("day", "")
            st.setdefault("training_start", time.strftime("%Y-%m-%d"))
            st["mode"] = cfg.get("mode")
            return st
    # vers begin (of oud/incompatibel formaat): elke strategievariant krijgt een
    # eigen nepgeld-portefeuille zodat ze eerlijk vergeleken kunnen worden
    variants = {v["naam"]: new_portfolio(cfg)
                for v in cfg.get("training", {}).get("varianten", [])} \
        or {"standaard": new_portfolio(cfg)}
    return {"day": "", "training_start": time.strftime("%Y-%m-%d"),
            "mode": cfg.get("mode"), "variants": variants}
...
def check_exits(eff, port, kp, market, live):
    """Alleen de VERKOOP-regels (stop-loss/take-profit/trailing/drempel). Los
    van kopen zodat we dit heel vaak kunnen draaien voor snelle uitgangen."""
    for mint, pos in list(port["positions"].items()):
        ...
        if trail and drop_from_peak <= -trail and pos["peak_price"] > pos["entry_price"]:
...
def run_rules(eff, port, kp, market, live):
    ...
    # 1) verkopen (stop-loss/take-profit/trailing/drempel)
    check_exits(eff, port, kp, market, live)
...
def fast_exit_check(cfg, st, kp, live):
    """Alleen de open posities snel bewaken (elke paar seconden) zodat verkopen
    binnen seconden gebeurt als een koers instort of doorschiet. Goedkoop:
    haalt alleen de prijzen van de tokens die je in bezit hebt."""
    held = set()
    for port in st["variants"].values():
        held.update(port["positions"].keys())
    if not held:
        return
    market = fetch_market(list(held))
    if not market:
        return
    variants = cfg.get("training", {}).get("varianten", [])
    if live or not variants:
        name = next(iter(st["variants"]))
        base = dict(cfg); base["_variant"] = ""
        check_exits(base, st["variants"][name], kp, market, live)
    else:
        for v in variants:
            port = st["variants"].get(v["naam"])
            if port and port["positions"]:
                check_exits(effective_cfg(cfg, v), port, kp, market, live)
...
def main():
    ...
    while running:
        ...
        # snelle-uitgang-bewaking tussen volle rondes door
        fast = max(2, int(cfg.get("snelle_uitgang_seconden", 4)))
        elapsed = 0
        interval = cfg.get("interval_seconden", 30)
        while running and elapsed < interval:
            ...
            if not running:
                break
            try:
                fast_exit_check(cfg, st, kp, live)
                save_state(st)
            except Exception as e:  # noqa: BLE001 — snelle check mag de bot niet stoppen
                log(f"snelle-uitgang overgeslagen: {e}")
